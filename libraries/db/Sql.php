...
class Sql
{
    ...
    {
        $sql = sprintf("select COUNT(*) as count from `%s` %s", $this->table, $this->filter);
    ...
    {
        $sql = sprintf("select %s from `%s` %s", $column, $this->table, $this->filter);
        $sth = Db::pdo()->prepare($sql);
        $sth = $this->formatParam($sth, $this->param);
        $sth->execute();
    ...
    public function deleteBy($column, $val)
    {
        $sql = sprintf("delete from `%s` where `%s` = :%s", $this->table, $column, $column);
        $sth = Db::pdo()->prepare($sql);
        $sth = $this->formatParam($sth, [$column => $val]);
        $sth->execute();
    ...
    // 新增数据
    public function add($data, $getInsertID = false)
    {
